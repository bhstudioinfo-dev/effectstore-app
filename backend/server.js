    ...
    wss.on('connection', (ws, request) => {
        ...
        ws.on('message', (raw) => {
            try {
                ...
                } else if (packet.event === 'gift_jar_reset' || packet.event === 'gift_jar_drop' || packet.event === 'gift_jar_bomb_drop' || packet.event === 'challenge_wheel_spin' || packet.event === 'vip_honor_playback_request') {
                    broadcastToClients(packet.event, packet.data || {});
                }
            } catch (error) {
